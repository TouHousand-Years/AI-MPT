...
- [Piano Roll Focus subordinate Tracker UI](issues/19-prototype-piano-roll-focus-subordinate-tracker.md): use a minimal event rail that retains row, playhead, channel-coloured events, shared cursor/selection, and inspectable Tracker-only markers without taking Piano Roll keyboard focus; raw details open read-only on demand, constrained widths compress the rail rather than hiding safety context, and the complete Tracker remains one immediate action away with shared state preserved.
...
- [Synchronized Piano Roll editing slice](issues/22-prototype-synchronized-piano-roll-editing-slice.md): the native Pattern Editor can host a recognizable time-horizontal Piano Roll projection over the active `CPattern`, share `PatternRect`, playback, native single-note mutation, and ordinary Pattern undo, and mark tracker-only content explicitly; the owner accepted the corrected no-flicker, non-obscuring layout and stronger selected-note treatment in the reviewed x64 Debug build.
