...
如果 Agent 切换到了一个已经存在、但当前 Sequence 没有引用的 Pattern，可以在同一保留会话中调用重排工具：目标数组继续包含全部原 Order 下标，并在需要的位置插入 `{pattern: N}`。这样既能指定准确的 Pattern 编号和播放位置，也不会丢失原有 Order 条目。只能补入确实未被当前 Sequence 引用的有效 Pattern；若 Pattern 已在 Order 中，应移动它现有的 Order 下标，而不是再次按 Pattern 编号插入。
