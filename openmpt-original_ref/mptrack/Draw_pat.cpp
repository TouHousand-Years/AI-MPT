...
// PROTOTYPE (Issue 22): Three native seams are deliberately kept visible here:
// CPattern is the only note store, PatternRect is the only selection, and the
// existing Pattern undo stack is the only history. This code is intentionally
// local and disposable; it is evidence for the production editor boundary.

CRect CViewPattern::GetPianoRollPrototypeRect() const
{
	CRect client;
	GetClientRect(&client);
	if(client.Width() < 860 || client.Height() < 360)
		return CRect{};
...
CRect CViewPattern::GetPianoRollPrototypeGridRect() const
{
	CRect grid = GetPianoRollPrototypeRect();
	if(grid.IsRectEmpty())
		return grid;
...
std::pair<int, int> CViewPattern::GetPianoRollPrototypePitchRange() const
{
	const CSoundFile *sndFile = GetSoundFile();
	if(sndFile == nullptr)
		return {NOTE_MIN, NOTE_MIN + 35};

	const auto &specs = sndFile->GetModSpecifications();
	const int minAllowed = specs.noteMin;
	...
	if(m_pianoRollPrototypeMinPitch < 0 && sndFile->Patterns.IsValidPat(m_nPattern))
	{
		...
		// follow-song advanced through a melody.
		const CPattern &pattern = sndFile->Patterns[m_nPattern];
		int lowest = maxAllowed;
		int highest = minAllowed;
		bool found = false;
		for(ROWINDEX row = 0; row < pattern.GetNumRows(); row++)
		{
			for(CHANNELINDEX channel = 0; channel < sndFile->GetNumChannels(); channel++)
			{
				const ModCommand &command = *pattern.GetpModCommand(row, channel);
				if(command.IsNote())
				{
					lowest = std::min(lowest, static_cast<int>(command.note));
					highest = std::max(highest, static_cast<int>(command.note));
					found = true;
				}
			}
		}
...
static CRect PianoRollPrototypeNoteRect(const CRect &grid, ROWINDEX row, ROWINDEX endRow, int note, int minPitch, int maxPitch, ROWINDEX numRows, CHANNELINDEX channel)
{
	const int pitchCount = maxPitch - minPitch + 1;
	const int x1 = grid.left + static_cast<int>((static_cast<int64>(row) * grid.Width()) / std::max<ROWINDEX>(1, numRows));
	const int x2 = grid.left + static_cast<int>((static_cast<int64>(endRow) * grid.Width()) / std::max<ROWINDEX>(1, numRows));
	const int y1 = grid.top + (maxPitch - note) * grid.Height() / pitchCount;
	const int y2 = grid.top + (maxPitch - note + 1) * grid.Height() / pitchCount;
	const int laneOffset = static_cast<int>(channel % 3);
	return CRect{x1 + 1, y1 + laneOffset, std::max(x1 + 9, x2 - 1), std::max(y1 + laneOffset + 5, y2 - 1)};
}


static bool PianoRollPrototypeIsBlackKey(int note)
{
	const int pitchClass = (note - NOTE_MIN) % 12;
	return pitchClass == 1 || pitchClass == 3 || pitchClass == 6 || pitchClass == 8 || pitchClass == 10;
}


void CViewPattern::DrawPianoRollPrototype(CDC &dc)
{
	...
	const auto [minPitch, maxPitch] = GetPianoRollPrototypePitchRange();
	const int pitchCount = maxPitch - minPitch + 1;

	const COLORREF paneColor = RGB(28, 31, 38);
	const COLORREF gridColor = RGB(21, 24, 30);
	const COLORREF strongGrid = RGB(62, 69, 82);
	const COLORREF weakGrid = RGB(40, 45, 54);
	const COLORREF whiteKeyRow = RGB(29, 33, 41);
	const COLORREF blackKeyRow = RGB(19, 22, 28);
	const COLORREF textColor = RGB(230, 234, 241);
	const COLORREF mutedText = RGB(157, 165, 180);
	const COLORREF warningColor = RGB(255, 176, 64);
	const COLORREF selectionColor = RGB(255, 224, 96);
	...
	CRect textRect = pane;
	textRect.DeflateRect(14, 8);
	textRect.bottom = textRect.top + 22;
	CString title;
	title.Format(_T("PIANO ROLL PROTOTYPE  |  Pattern %u  |  native data"), m_nPattern);
	dc.DrawText(title, textRect, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
	...
	const CRect upButton{downButton.right + 6, buttonTop, downButton.right + 64, buttonTop + 28};
	const CRect undoButton{upButton.right + 10, buttonTop, upButton.right + 76, buttonTop + 28};
	const CRect playButton{undoButton.right + 6, buttonTop, undoButton.right + 106, buttonTop + 28};
	const auto drawButton = [&dc, textColor](const CRect &rect, const CString &label, bool enabled)
	{
		dc.FillSolidRect(rect, enabled ? RGB(55, 62, 75) : RGB(40, 43, 50));
		dc.Draw3dRect(rect, enabled ? RGB(116, 127, 149) : RGB(64, 68, 78), RGB(13, 15, 19));
		dc.SetTextColor(enabled ? textColor : RGB(112, 117, 127));
		CRect labelRect = rect;
		dc.DrawText(label, labelRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
	};
	drawButton(downButton, _T("-1"), GetCursorCommand().IsNote());
	...
	dc.FillSolidRect(grid, gridColor);
	dc.FillSolidRect(keyboard, RGB(214, 218, 225));
	dc.FillSolidRect(timeRuler, RGB(35, 39, 48));

	// A real pitch keyboard and alternating pitch lanes are essential evidence:
	// without them a sparse native Pattern reads as an empty debug grid rather
	// than a Piano Roll.
	for(int note = minPitch; note <= maxPitch; note++)
	{
		const int y1 = grid.top + (maxPitch - note) * grid.Height() / pitchCount;
		const int y2 = grid.top + (maxPitch - note + 1) * grid.Height() / pitchCount;
		const bool blackKey = PianoRollPrototypeIsBlackKey(note);
		dc.FillSolidRect(CRect{grid.left, y1, grid.right, y2}, blackKey ? blackKeyRow : whiteKeyRow);

		CRect keyRect{keyboard.left, y1, keyboard.right, y2};
		if(blackKey)
		{
			keyRect.left += keyboard.Width() / 3;
			dc.FillSolidRect(keyRect, RGB(35, 38, 45));
		} else
		{
			dc.FillSolidRect(keyRect, RGB(224, 227, 233));
		}
		dc.Draw3dRect(keyRect, blackKey ? RGB(72, 77, 88) : RGB(250, 250, 252), RGB(92, 98, 111));

		if(((note - NOTE_MIN) % 12) == 0)
		{
			CString noteName = mpt::ToCString(sndFile.GetNoteName(static_cast<ModCommand::NOTE>(note), static_cast<INSTRUMENTINDEX>(0)));
			dc.SetTextColor(RGB(39, 43, 51));
			CRect labelRect = keyRect;
			labelRect.DeflateRect(5, 0);
			dc.DrawText(noteName, labelRect, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
		}
	}
	...
	for(ROWINDEX row = 0; row <= numRows; row++)
	{
		...
		dc.LineTo(x, grid.bottom);
		dc.SelectObject(oldPen);
		if(row != numRows && row % 16 == 0)
		{
			CString rowLabel;
			rowLabel.Format(_T("%u"), row);
			dc.SetTextColor(mutedText);
			CRect rowLabelRect{x + 3, timeRuler.top, std::min<LONG>(x + 46, timeRuler.right), timeRuler.bottom};
			dc.DrawText(rowLabel, rowLabelRect, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
		}
	}
	for(int note = minPitch; note <= maxPitch; note++)
	{
		const int y = grid.top + (maxPitch - note) * grid.Height() / pitchCount;
		const bool octave = ((note - NOTE_MIN) % 12) == 0;
		CPen pen(PS_SOLID, 1, octave ? strongGrid : weakGrid);
		CPen *oldPen = dc.SelectObject(&pen);
		dc.MoveTo(grid.left, y);
		dc.LineTo(grid.right, y);
		dc.SelectObject(oldPen);
	}

	static constexpr COLORREF channelColors[] =
	{
	...
	for(CHANNELINDEX channel = 0; channel < sndFile.GetNumChannels(); channel++)
	{
		...
		{
			const ModCommand &command = *pattern.GetpModCommand(row, channel);
			const bool hasTrackerDetail = command.IsPcNote() || command.volcmd != VOLCMD_NONE || command.command != CMD_NONE;
			if(hasTrackerDetail)
			{
				detailMarkers++;
				const int x = time.left + static_cast<int>((static_cast<int64>(row) * time.Width()) / std::max<ROWINDEX>(1, numRows));
				...
				dc.Polygon(diamond, static_cast<int>(std::size(diamond)));
			...
			if(command.IsSpecialNote() && !command.IsPcNote())
			{
				...
				dc.MoveTo(x, grid.top);
				dc.LineTo(x, grid.top + 14 + static_cast<int>(channel % 3) * 4);
				dc.SelectObject(oldPen);
				continue;
			}
			if(!command.IsNote() || command.note < minPitch || command.note > maxPitch)
				continue;

			ROWINDEX endRow = numRows;
			bool explicitEnd = false;
			for(ROWINDEX nextRow = row + 1; nextRow < numRows; nextRow++)
			{
				const ModCommand &next = *pattern.GetpModCommand(nextRow, channel);
				if(next.note != NOTE_NONE)
				{
					endRow = nextRow;
					explicitEnd = next.IsSpecialNote() && !next.IsPcNote();
					break;
				}
			}
			...
			const bool selected = m_Selection.Contains(PatternCursor(row, channel, PatternCursor::noteColumn));
			if(selected)
			{
				// Selection must survive peripheral vision during playback. A broad
				// gold halo, bright fill and dark grab handle are deliberately much
				// stronger than the old one-pixel yellow outline.
				CRect halo = noteRect;
				halo.InflateRect(3, 2);
				dc.FillSolidRect(halo, selectionColor);
				dc.FillSolidRect(noteRect, RGB(255, 244, 174));
				dc.Draw3dRect(noteRect, RGB(255, 255, 245), RGB(194, 139, 24));
				const CRect handle{noteRect.left + 2, noteRect.top + 2, std::min(noteRect.left + 8, noteRect.right - 1), noteRect.bottom - 2};
				if(handle.right > handle.left && handle.bottom > handle.top)
					dc.FillSolidRect(handle, RGB(77, 58, 13));
			} else
			{
				dc.FillSolidRect(noteRect, noteColor);
				CBrush frameBrush(RGB(11, 13, 17));
				dc.FrameRect(noteRect, &frameBrush);
			}
			if(!explicitEnd)
			{
				CPen implicitPen(PS_DOT, 1, RGB(225, 230, 240));
				CPen *oldPen = dc.SelectObject(&implicitPen);
				dc.MoveTo(noteRect.right - 1, noteRect.top + 1);
				dc.LineTo(noteRect.right - 1, noteRect.bottom - 1);
				dc.SelectObject(oldPen);
			}
	...
	state.Format(_T("Rows %u-%u | Ch %u-%u | cursor %u/%u | %s"),
		m_Selection.GetStartRow(), m_Selection.GetEndRow(), m_Selection.GetStartChannel() + 1,
		m_Selection.GetEndChannel() + 1, GetCurrentRow(), GetCurrentChannel() + 1,
		m_pianoRollPrototypeStatus.GetString());
	dc.DrawText(state, stateRect, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
...
void CViewPattern::InvalidatePianoRollPrototype()
{
...
bool CViewPattern::TransposePianoRollPrototypeNote(int semitones)
{
	...
	if(sndFile == nullptr || !sndFile->Patterns.IsValidPat(m_nPattern))
		return false;
	ModCommand &command = GetCursorCommand();
	if(!command.IsNote())
	{
		m_pianoRollPrototypeStatus = _T("No pitched native note is selected.");
		InvalidatePianoRollPrototype();
		return false;
	}
	const auto &specs = sndFile->GetModSpecifications();
	const int newNote = Clamp(static_cast<int>(command.note) + semitones, static_cast<int>(specs.noteMin), static_cast<int>(specs.noteMax));
	if(newNote == command.note)
		return false;
	const PatternCursor cursor = m_Cursor;
	if(!PrepareUndo(cursor, cursor, "Piano Roll Prototype: Transpose Note"))
		return false;
	command.note = static_cast<ModCommand::NOTE>(newNote);
	SetModified();
	SetCursorPosition(cursor);
	PreviewNote(cursor.GetRow(), cursor.GetChannel());
	m_pianoRollPrototypeStatus.Format(_T("Committed %s at row %u, channel %u; ordinary Undo is ready."),
		mpt::ToCString(sndFile->GetNoteName(command.note, command.instr)).GetString(), cursor.GetRow(), cursor.GetChannel() + 1);
	InvalidatePianoRollPrototype();
	return true;
}


bool CViewPattern::HandlePianoRollPrototypeLButtonDown(UINT, CPoint point)
{
	const CRect pane = GetPianoRollPrototypeRect();
	const CRect grid = GetPianoRollPrototypeGridRect();
	const CRect time = GetPianoRollPrototypeTimeRect();
	...
	const CRect undoButton{upButton.right + 10, buttonTop, upButton.right + 76, buttonTop + 28};
	const CRect playButton{undoButton.right + 6, buttonTop, undoButton.right + 106, buttonTop + 28};
	if(downButton.PtInRect(point))
	{
		TransposePianoRollPrototypeNote(-1);
		return true;
	}
	if(upButton.PtInRect(point))
	{
		TransposePianoRollPrototypeNote(1);
		return true;
	}
	if(undoButton.PtInRect(point))
	{
		OnEditUndo();
		m_pianoRollPrototypeStatus = _T("Ordinary Pattern Undo requested.");
		InvalidatePianoRollPrototype();
		return true;
	}
	if(playButton.PtInRect(point))
	{
		GetDocument()->OnPatternPlay();
		m_pianoRollPrototypeStatus = _T("Playing through OpenMPT's normal Pattern playback.");
		InvalidatePianoRollPrototype();
		return true;
	}
	if(!grid.PtInRect(point))
		return true;
	...
	for(CHANNELINDEX channel = 0; channel < sndFile->GetNumChannels(); channel++)
	{
		for(ROWINDEX row = 0; row < numRows; row++)
		{
			...
			for(ROWINDEX nextRow = row + 1; nextRow < numRows; nextRow++)
			{
				if(pattern.GetpModCommand(nextRow, channel)->note != NOTE_NONE)
				{
					endRow = nextRow;
					break;
				}
			...
			if(noteRect.PtInRect(point))
			{
				...
				SetCursorPosition(cursor);
				SetCurSel(cursor);
				m_pianoRollPrototypeDragging = true;
				m_pianoRollPrototypeDragRow = row;
				m_pianoRollPrototypeDragChannel = channel;
				m_pianoRollPrototypeDragNote = command.note;
				m_pianoRollPrototypePreviewNote = command.note;
				m_pianoRollPrototypeStatus.Format(_T("Selected row %u, channel %u through PatternRect."), row, channel + 1);
				SetCapture();
				InvalidatePianoRollPrototype();
				return true;
			}
	...
	SetCursorPosition(cursor);
	SetCurSel(cursor);
	m_pianoRollPrototypeStatus.Format(_T("Navigated to empty row %u, channel %u; no Pattern data changed."), row, channel + 1);
	InvalidatePianoRollPrototype();
	return true;
}


bool CViewPattern::UpdatePianoRollPrototypeDrag(CPoint point)
{
	if(!m_pianoRollPrototypeDragging)
		return false;
	const CRect grid = GetPianoRollPrototypeGridRect();
	const auto [minPitch, maxPitch] = GetPianoRollPrototypePitchRange();
	const int pitchCount = maxPitch - minPitch + 1;
	const int y = Clamp(point.y, grid.top, grid.bottom - 1);
	const int note = maxPitch - ((y - grid.top) * pitchCount) / std::max(1, grid.Height());
	m_pianoRollPrototypePreviewNote = static_cast<ModCommand::NOTE>(Clamp(note, minPitch, maxPitch));
	m_pianoRollPrototypeStatus.Format(_T("Preview transpose: %+d semitone%s (release to commit)."),
		static_cast<int>(m_pianoRollPrototypePreviewNote) - static_cast<int>(m_pianoRollPrototypeDragNote),
		std::abs(static_cast<int>(m_pianoRollPrototypePreviewNote) - static_cast<int>(m_pianoRollPrototypeDragNote)) == 1 ? _T("") : _T("s"));
	InvalidatePianoRollPrototype();
	return true;
}


bool CViewPattern::FinishPianoRollPrototypeDrag(CPoint point)
{
	if(!m_pianoRollPrototypeDragging)
		return false;
	UpdatePianoRollPrototypeDrag(point);
	if(GetCapture() == this)
		ReleaseCapture();
	const int semitones = static_cast<int>(m_pianoRollPrototypePreviewNote) - static_cast<int>(m_pianoRollPrototypeDragNote);
	const PatternCursor cursor(m_pianoRollPrototypeDragRow, m_pianoRollPrototypeDragChannel, PatternCursor::noteColumn);
	m_pianoRollPrototypeDragging = false;
	SetCursorPosition(cursor);
	if(semitones != 0)
		TransposePianoRollPrototypeNote(semitones);
	else
		m_pianoRollPrototypeStatus = _T("Selection committed through PatternRect; note was not changed.");
	m_pianoRollPrototypePreviewNote = NOTE_NONE;
	InvalidatePianoRollPrototype();
	return true;
}
...
void CViewPattern::UpdateScrollSize()
{
	...
	// The dedicated prototype pane reserves the right side of the Pattern view.
	// Add its width to the Tracker scroll extent so every native channel remains
	// reachable within the smaller left-hand viewport.
	sizeTotal.cx += GetPianoRollPrototypeRect().Width();
...
BOOL CViewPattern::OnScrollBy(CSize sizeScroll, BOOL bDoScroll)
{
	...
	if ((x != m_nXScroll) || (y != m_nYScroll))
	{
		CRect rect;
		GetClientRect(&rect);
		const CRect pianoRollPane = GetPianoRollPrototypeRect();
		const bool hasPianoRollPane = !pianoRollPane.IsRectEmpty();
		// HACK:
		...
		// The prototype is a fixed synchronized pane, not Pattern scroll
		// content. Repaint it as one unit after Tracker scrolling so no partial
		// invalidation can leave moved text, notes, or grid fragments behind.
		if(hasPianoRollPane)
			InvalidateRect(&pianoRollPane, FALSE);
	}
