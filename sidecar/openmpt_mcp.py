...
TOOLS = [
    ...
    {"name": "replace_pattern_segment", "description": "Replace one contiguous segment of one channel in the private candidate. Indices are zero-based; omitted rows become complete empty cells. Pitched notes and the special note IDs published in context.format.special_notes are writable. All format-supported volume and effect commands are editable; use the command IDs and parameter ranges published by get_pattern_context. Preserve unsupported raw values unless replacing them with a supported value. Expansion can wait for human approval. " + ENDING,
